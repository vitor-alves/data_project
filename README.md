...
3. **Develop a way to obtain the weekly average number of trips for an area, defined by a bounding box (given by coordinates) or by a region.**
    * Assuming a bounding box similar to the drawing below, with points (x1,y1), (x2,y2), (x3,y3), (x4,y4), you can calculate the weekly average number of trips with a simple SQL query, stored in sql_queries/weekly_average_num_trips.sql. To check the query, please open that file.
